 ...
 */
export function collectSkillsForList(files, pendingBaseSkill, instructionsFile, sourceTarget, repoPath) {
  const baseSkillPrefix = sourceTarget.skillsDir + '/base/';
...
function buildRootInstructions(baseSkill, instructionsFile, domainSkills, graphSerialized, destTarget) {
  ...
  // No fallback to baseSkill content when instructionsFile is unavailable: a
  // skill file's own content (frontmatter and all) is not a substitute for
  // root instructions prose. During doc-init's chunked incremental writes,
  // the root instructions file doesn't exist yet while the base/domain
  // skills are still generating — falling back to baseSkill.content here
  // used to write the base skill's raw content as the root AGENTS.md/CLAUDE.md,
  // and since that first incremental write isn't forced, the later correct
  // write (once the real instructions file exists) got silently skipped as
  // "already exists". Omitting the section here means transformToDirectoryScoped
  // simply doesn't emit a root file yet, which is correct — there's nothing
  // real to write until instructionsFile exists.
  if (instructionsFile) {
...
// Path to the aspens-owned index file (Claude target only — see
// ensureAspensImportBlock). Safe to regenerate wholesale every run.
export const ASPENS_INDEX_PATH = '.claude/aspens-index.md';

const ASPENS_BLOCK_START = '<!-- aspens:start -->';
const ASPENS_BLOCK_END = '<!-- aspens:end -->';

/**
 * Build the content of the aspens-owned index file: the Skills list and the
 * deterministic Behavior guardrails. This file is regenerated wholesale on
 * every run (aspens fully owns it), so it's safe to overwrite without any
 * preservation logic — unlike CLAUDE.md, which may carry substantial
 * hand-authored content the user owns.
 */
export function buildAspensIndexContent(baseSkill, domainSkills, destTarget, hasArchitectureSkill = false) {
  let content = '# Aspens Index\n\nAuto-generated by aspens — regenerated wholesale on every run. Do not hand-edit; edits will be overwritten. Imported from the root instructions file via a delimited `aspens:start`/`aspens:end` block.\n';
  content = syncSkillsSection(content, baseSkill, domainSkills, destTarget, hasArchitectureSkill);
  content = syncBehaviorSection(content);
  return content;
}

/**
 * Maintain a single delimited, tool-owned block in a root instructions file
 * (CLAUDE.md) that imports the aspens-owned index file via Claude Code's
 * `@path` memory-import syntax — the same mechanism a repo's own CLAUDE.md
 * commonly uses for `@AGENTS.md` (see this workspace's `~/rules/CLAUDE.repo.md`).
 *
 * Only ever replaces content between the markers; never touches anything
 * outside them; appends the block at the end of the file (never inserts
 * mid-document) when the markers aren't present yet. This replaces the older
 * approach of injecting `## Skills`/`## Behavior` content directly into the
 * file, which had no way to distinguish "content aspens generated" from
 * "content the user wrote" once both lived in the same untagged document —
 * any pre-existing `## Skills`/`## Behavior` heading from that older
...
function logicalKeyForFile(filePath, target) {
  // The aspens-owned index file is a Claude-only auxiliary (imported via a
  // `@path` block in CLAUDE.md) with no working equivalent on codex/opencode
  // — exclude it from cross-target parity checks by design.
  if (filePath === ASPENS_INDEX_PATH) return null;
